...
expectError(expect(mockClient(SNSClient)).toHaveReceivedCommandTimes(PublishCommand, 1))
...
expect(mockClient(SNSClient)).toHaveNthReceivedCommandWith(1, PublishCommand, {Message: ''})
expectError(expect(mockClient(SNSClient)).toHaveNthReceivedCommandWith(1, PublishCommand, { Foo: '' }))